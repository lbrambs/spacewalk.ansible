...
Is it an issue? Well no but it is confusing, spacewalk will correctly update the packages but for the client it looks like the packages aren't updated (if you check with apt). Verify with other means (logs, dpkg) to see that the packages are indeed updated.
